...
func Test_reference_document(t *testing.T) {
	var (
		...
		order   = design.NewRecord(Order{})
	)

	graph := design.NewGraph()
	graph.Place(account).At(30, 60)
...
can be mixed with types, E.g. `,
		...
		ul(
			...
			"Toggle methods",
			"Sortable fields",
			"Allow for arbitrary links",
			"Expandable diagrams",
			"Quick overview generator (perhaps)",
			"Nice links to godoc",
		),
		"<hr><i>This document is generated by func ",
